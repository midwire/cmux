...
#
# Commands:
#   cmux new <branch> [--from <base>] [-p <prompt>]   — New worktree + branch, run setup hook, launch Claude
...
cmux() {
  ...
  case "$cmd" in
    ...
    --help|-h|"")
      ...
      echo ""
      echo "  new <branch> [--from <base>] [-p <prompt>]"
...
_cmux_new() {
  if [[ "$1" == "--help" || "$1" == "-h" ]]; then
    echo "Usage: cmux new <branch> [--from <base>] [-p <prompt>]"
    echo ""
